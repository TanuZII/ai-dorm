...
import { calculateReportTotals } from './reportTotals'
...
function Reports({ notify }) {
  ...
  const visibleTotals = useMemo(() => calculateReportTotals(report?.columns || [], rows), [report, rows])
  ...
  return <div className="space-y-5">
    ...
    <div className="grid gap-5 xl:grid-cols-[280px_minmax(0,1fr)]">
      ...
      <section className="min-w-0 overflow-hidden rounded-2xl border border-[#dce6ea] bg-white shadow-sm">
        ...
        {loading ? <div className="grid min-h-80 place-items-center"><LoaderCircle className="animate-spin text-[#0f766e]"/></div> : <div className="overflow-auto"><table className="min-w-full whitespace-nowrap text-left"><thead className="sticky top-0 z-10 bg-[#f0f5f6] text-[9px] font-semibold text-[#405c70]"><tr>{report?.columns.map(column => <th key={column.key} className={`px-4 py-3 ${['money','number','percentage'].includes(column.type) ? 'text-right' : ''}`}><button onClick={() => changeSort(column.key)} className="inline-flex items-center gap-1 hover:text-[#0f766e]">{column.label}{sort.key === column.key && (sort.direction === 'asc' ? <ArrowDownAZ size={12}/> : <ArrowUpAZ size={12}/>)}</button></th>)}</tr></thead><tbody className="divide-y divide-[#edf1f3] text-[10px] text-[#395368]">{rows.map((row, rowIndex) => <tr key={`${rowIndex}-${Object.values(row)[0]}`} className="hover:bg-[#f8fbfa]">{report.columns.map(column => <td key={column.key} className={`px-4 py-3 ${['money','number','percentage'].includes(column.type) ? "text-right font-['IBM_Plex_Sans'] tabular-nums" : ''}`}>{column.type === 'money' ? money(row[column.key]) : column.type === 'percentage' ? `${row[column.key] ?? 0}%` : row[column.key] ?? '-'}</td>)}</tr>)}{rows.length === 0 && <tr><td colSpan={report?.columns.length || 1} className="h-48 text-center text-xs text-[#8b9aa4]">ไม่พบข้อมูลในช่วงที่เลือก</td></tr>}</tbody>{rows.length>0&&<tfoot className="bg-[#e8f5f1] text-[10px] font-semibold text-[#0f665b]"><tr>{report.columns.map((column,index)=><td key={column.key} className={`px-4 py-3 ${['money','number'].includes(column.type)?'text-right':''}`}>{index===0?'รวม':column.type==='money'?money(visibleTotals[column.key]):column.type==='number'?(visibleTotals[column.key]||0).toLocaleString('th-TH'):''}</td>)}</tr></tfoot>}</table></div>}
      </section>
    </div>
  </div>
}
