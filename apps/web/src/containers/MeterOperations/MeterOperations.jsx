...
import { activeUtilityRate } from './utilityRates'
...
const initialForm = { roomId: '', utilityType: 'electricity', billingMonth: '', previousReading: '', currentReading: '', dueDate: '' }
...
function MeterOperations({ notify }) {
  ...
  const load = async () => {
    setLoading(true)
    try { const [roomRows, meterRows, rates] = await Promise.all([api('/rooms'), api('/meter-readings'), api('/utility-rates')]); setRooms(roomRows); setMeters(meterRows); setUtilityRates(rates) }
    catch (error) { notify(error.message) }
    finally { setLoading(false) }
  }
  useEffect(() => { load() }, [])
  const selectedRate = useMemo(() => activeUtilityRate(utilityRates, form.utilityType, form.billingMonth), [utilityRates, form.utilityType, form.billingMonth])
  const update = (key, value) => setForm(current => ({ ...current, [key]: value }))
  const submit = async event => {
    event.preventDefault(); setSaving(true)
    try {
      if (!selectedRate) throw new Error('ไม่พบอัตราค่าสาธารณูปโภคที่มีผลสำหรับเดือนนี้ กรุณาตั้งค่า Config ก่อน')
      await api('/meter-readings', { method: 'POST', body: { roomId: Number(form.roomId), utilityType: form.utilityType, billingMonth: form.billingMonth, previousReading: Number(form.previousReading), currentReading: Number(form.currentReading), dueDate: form.dueDate, issueInvoices: true } })
  ...
  return <div className="enter-up space-y-5">
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1.2fr)_380px]">
      ...
      <form onSubmit={submit} className="rounded-2xl border border-[#dfe7eb] bg-white p-5"><div className="mb-5"><p className="text-[10px] font-semibold text-[#397caf]">บันทึกรอบใหม่</p><h3 className="mt-1 text-lg font-semibold">จดมิเตอร์และตั้งหนี้</h3></div><div className="grid gap-4"><Field label="ห้องพัก"><select required value={form.roomId} onChange={e=>update('roomId',e.target.value)} className={fieldClass}><option value="">เลือกอาคารและห้อง</option>{rooms.map(x=><option key={x.id} value={x.id}>{x.building_name} · ชั้น {x.floor_no} · ห้อง {x.room_no}</option>)}</select></Field><Field label="ประเภทมิเตอร์"><select value={form.utilityType} onChange={e=>update('utilityType',e.target.value)} className={fieldClass}><option value="electricity">ไฟฟ้า</option><option value="water">น้ำประปา</option></select></Field><Field label="รอบเดือน"><input required type="month" value={form.billingMonth} onChange={e=>update('billingMonth',e.target.value)} className={fieldClass}/></Field><div className={`rounded-xl border p-3 text-[11px] ${selectedRate?'border-[#b9d9ec] bg-[#f0f8fd] text-[#276d9d]':'border-[#efc6c1] bg-[#fff2f0] text-[#b74e44]'}`}>{selectedRate ? <>อัตราจาก Config: <b>{Number(selectedRate.unit_rate).toLocaleString('th-TH')} บาท/หน่วย</b>{selectedRate.minimum_charge > 0 && <> · ขั้นต่ำ {Number(selectedRate.minimum_charge).toLocaleString('th-TH')} บาท</>}</> : 'เลือกเดือนเพื่อค้นหาอัตราที่มีผลจาก Config'}</div><div className="grid grid-cols-2 gap-3"><Field label="เลขครั้งก่อน"><input required type="number" min="0" step="0.01" value={form.previousReading} onChange={e=>update('previousReading',e.target.value)} className={fieldClass}/></Field><Field label="เลขปัจจุบัน"><input required type="number" min="0" step="0.01" value={form.currentReading} onChange={e=>update('currentReading',e.target.value)} className={fieldClass}/></Field></div><Field label="วันครบกำหนด"><input required type="date" value={form.dueDate} onChange={e=>update('dueDate',e.target.value)} className={fieldClass}/></Field></div><button disabled={saving || !selectedRate} className="mt-5 flex w-full items-center justify-center gap-2 rounded-xl bg-[#f5bf3c] py-3 text-xs font-semibold text-[#173653] shadow-[0_3px_0_#c28d15] disabled:opacity-60">{saving&&<LoaderCircle size={15} className="animate-spin"/>} บันทึกมิเตอร์และออกหนี้</button></form>
    </div>
  </div>
}
