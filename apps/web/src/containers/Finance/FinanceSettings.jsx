...
import { CalendarRange, Droplets, LoaderCircle, Plus, ReceiptText, X, Zap } from 'lucide-react'
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-5 xl:grid-cols-2">
      ...
      <RateSection title="ค่าธรรมเนียมอื่น ๆ" detail="ค่าเริ่มต้นสำหรับการตั้งหนี้รายบุคคล" icon={ReceiptText} onAdd={canManage ? () => setModal('fee') : null}>
        <Table headers={['รหัส', 'ชื่อรายการ', 'ค่าเริ่มต้น']}>
          {data.fees.map(row => <tr key={row.id}><Cell><b>{row.code}</b></Cell><Cell>{row.name}</Cell><Cell><b>{money(row.default_amount)} บาท</b></Cell></tr>)}
        </Table><Empty rows={data.fees} />
      </RateSection>
    </div>
    {modal && <SettingsDialog type={modal} close={() => setModal(null)} save={save} />}
  </div>
}
...
function SettingsDialog({ type, close, save }) {
  const [saving, setSaving] = useState(false)
  const title = { rate: 'เพิ่มอัตราค่าห้องพัก', utility: 'เพิ่มอัตราค่าสาธารณูปโภค', fee: 'เพิ่มค่าธรรมเนียม' }[type]
  ...
  return <div className="fixed inset-0 z-50 grid place-items-end bg-[#142a46]/45 sm:place-items-center sm:p-4" onMouseDown={event => event.target === event.currentTarget && close()}><form onSubmit={submit} className="max-h-[92vh] w-full max-w-lg overflow-y-auto rounded-t-3xl bg-white shadow-2xl sm:rounded-3xl"><div className="flex items-center border-b border-[#e5ebef] p-5"><div><h3 className="text-base font-semibold">{title}</h3><p className="mt-1 text-[10px] text-[#7d8e9b]">การเพิ่มอัตราใหม่จะไม่แก้ไขประวัติอัตราเดิม</p></div><button type="button" onClick={close} className="ml-auto grid size-9 place-items-center rounded-full bg-[#f2f5f7]"><X size={17} /></button></div><div className="grid gap-4 p-5">
    ...
    {type === 'fee' && <><Field label="รหัสค่าธรรมเนียม"><input required name="code" className={fieldClass} placeholder="เช่น DAMAGE" /></Field><Field label="ชื่อรายการ"><input required name="name" className={fieldClass} placeholder="เช่น ค่าปรับทรัพย์สินเสียหาย" /></Field><Field label="จำนวนเงินเริ่มต้น"><input required min="0" step="0.01" type="number" name="defaultAmount" defaultValue="0" className={fieldClass} /></Field></>}
    {type !== 'fee' && <div className="grid grid-cols-2 gap-3"><Field label="วันที่เริ่มใช้"><input required type="date" name="startsAt" className={fieldClass} /></Field><Field label="วันที่สิ้นสุด (ถ้ามี)"><input type="date" name="endsAt" className={fieldClass} /></Field></div>}
