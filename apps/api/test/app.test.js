...
test('critical dormitory backend flows', async (t) => {
  ...
  await t.test('inventory cannot be overdrawn', async () => {
    ...
    const openingMovements = db.prepare(`SELECT movement_type,quantity,reference FROM inventory_movements WHERE item_id=? ORDER BY id`).all(item.body.id).map(row => ({ ...row }))
    assert.deepEqual(openingMovements, [{ movement_type: 'adjust', quantity: 10, reference: 'ยอดเริ่มต้น' }])
  ...
  await t.test('general reports calculate remittance rules and export sortable Excel', async () => {
    ...
    const halfPolicy=await api('/revenue-share-policies',{method:'POST',body:{code:'DORM_ROOM_HALF',itemType:'room',revenueGroup:'รายได้หอพัก',revenueName:'ค่าห้องพัก',reclaimRate:0.5,universityRate:0.5,startsAt:'2026-10-10'}})
    assert.equal(halfPolicy.response.status,201)
    const tinyInvoice=await issueConfiguredInvoice(tenantId,'2026-10-20',[{itemType:'room',description:'ค่าห้องพักหนึ่งสตางค์',quantity:1,unitPrice:0.01}])
    assert.equal(tinyInvoice.response.status,201)
    const tinyPayment=await api('/payments',{method:'POST',body:{invoiceId:tinyInvoice.body.id,amount:0.01,method:'transfer',referenceNo:'ROUNDING-001',paidAt:'2026-10-20T09:00:00.000Z'}})
    assert.equal(tinyPayment.response.status,201)
    const cancellablePolicy=await api('/revenue-share-policies',{method:'POST',body:{code:'DORM_ROOM_CANCELLED',itemType:'room',revenueGroup:'รายได้หอพัก',revenueName:'ค่าห้องพัก',reclaimRate:0.6,universityRate:0.4,startsAt:'2026-10-15'}})
    assert.equal(cancellablePolicy.response.status,201)
    const cancelledPolicy=await api(`/revenue-share-policies/${cancellablePolicy.body.id}`,{method:'PATCH',body:{active:false,reason:'ยกเลิกก่อนเริ่มใช้นโยบาย'}})
    assert.equal(cancelledPolicy.response.status,200)
    assert.equal(cancelledPolicy.body.active,0)
    assert.equal(cancelledPolicy.body.ends_at,null)
    const tinyRemittance=await api('/reports/general?type=revenue-remittance&from=2026-10-20&to=2026-10-20')
    const tinyRow=tinyRemittance.body.rows.find(row=>row.policy_code==='DORM_ROOM_HALF')
    assert.ok(!tinyRemittance.body.rows.some(row=>row.policy_code==='DORM_ROOM_CANCELLED'))
