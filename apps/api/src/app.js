...
import { createStudentDirectory } from './integrations/studentDirectory.js'
import { financialItemTypes } from './financeTypes.js'
import { bangkokDate } from './businessDate.js'
...
function contractPricingFromReservation(db, tenant, bed, rentalPeriod) {
  const reservation = db.prepare(`SELECT * FROM reservations
    WHERE tenant_id=? AND room_id=? AND status IN ('reserved','checked_in')
      AND (bed_id IS NULL OR bed_id=?) ORDER BY id DESC LIMIT 1`).get(tenant.id, bed.room_id, bed.id)
  if (!reservation?.condition_snapshot) throw httpError(409,'RESERVATION_RATE_MISSING','ไม่พบเงื่อนไขราคาจากการจอง กรุณาจองห้องหรือเตียงใหม่จากอัตราในเฟส 1')
  let snapshot
  try { snapshot = JSON.parse(reservation.condition_snapshot) } catch { throw httpError(409,'RESERVATION_RATE_INVALID','ข้อมูลอัตราจากการจองไม่สมบูรณ์') }
  if (snapshot.rentalPeriod !== rentalPeriod) throw httpError(409,'RENTAL_PERIOD_MISMATCH','รอบการเช่าของสัญญาต้องตรงกับการจอง')
  return { ...snapshot, reservationId: reservation.id, reservationNo: reservation.reservation_no }
}

function currentContractPricing(db, tenant, rentalPeriod, startsAt, rateScope, preferredCode = null) {
  const cohort = tenantRateCohort(tenant)
  const summerClass = preferredCode?.includes('SUMMER') ? 1 : 0
  const policy = db.prepare(`SELECT * FROM rate_policies WHERE tenant_cohort=? AND rental_period=? AND rate_scope=?
    AND active=1 AND starts_at<=? AND (ends_at IS NULL OR ends_at>=?)
    AND (? IS NULL OR CASE WHEN code LIKE '%SUMMER%' THEN 1 ELSE 0 END=?)
    ORDER BY starts_at DESC,id DESC LIMIT 1`)
    .get(cohort,rentalPeriod,rateScope,startsAt,startsAt,preferredCode,summerClass)
  if (!policy) throw httpError(409,'RATE_POLICY_NOT_CONFIGURED','ไม่พบนโยบายค่าเช่าที่มีผลสำหรับการต่อสัญญา')
  return { tenantType:tenant.tenant_type,cohort,rentalPeriod,rateScope,ratePolicyCode:policy.code,amount:policy.amount,occupancyLimit:policy.occupancy_limit,utilitySplitDivisor:policy.utility_split_divisor,depositAmount:policy.deposit_amount,dueDay:policy.due_day,lateFee:policy.late_fee }
}
...
function contractTypeForTenant(tenant, rentalPeriod) {
  if (tenant.tenant_type === 'staff') return 'STAFF_MONTH'
  if (tenant.tenant_type === 'external') return 'EXTERNAL_MONTH'
  return rentalPeriod === 'monthly' ? 'STUDENT_MONTH' : 'STUDENT_TERM'
}
...
function createContractInvoice(db, contract, createdBy) {
  const existing = db.prepare(`SELECT * FROM invoices WHERE contract_id=? AND source_type='contract'`).get(contract.id)
  ...
  const result = db.prepare(`INSERT INTO invoices(invoice_no,tenant_id,due_date,total,balance,contract_id,source_type,source_id,created_by) VALUES (?,?,?,?,?,?,'contract',?,?)`)
    .run(nextDocumentNo(db,'invoices','invoice_no','INV'),contract.tenant_id,contract.starts_at,total,total,contract.id,contract.id,createdBy)
...
function signedContractForCharge(db, tenantId, chargeDate) {
  return db.prepare(`SELECT * FROM leases WHERE tenant_id=? AND document_status='signed' AND status!='cancelled' AND date(?) BETWEEN date(starts_at) AND date(ends_at) ORDER BY signed_at DESC,id DESC LIMIT 1`).get(tenantId,chargeDate)
}

function contractFinanceRules(contract, tenantType, feeDefault = 100) {
  let snapshot = {}; try { snapshot=JSON.parse(contract?.rate_snapshot_json||'{}') } catch {}
  return {
    lateFee:Number(snapshot.lateFee ?? feeDefault),
    delinquencyMonths:Number(snapshot.delinquencyMonths ?? (tenantType==='student'?2:1)),
    terminationAction:snapshot.terminationAction || (tenantType==='student'?'ยุติการเช่าห้องพัก':'มหาวิทยาลัยมีสิทธิ์บอกเลิกสัญญาเช่า'),
  }
}
...
function renewalAlertDate(endsAt, rentalPeriod) {
  ...
  const months = rentalPeriod === 'term' ? 1 : rentalPeriod === 'yearly' ? 2 : 0
  if (months) {
    const firstOfTarget = new Date(Date.UTC(end.getUTCFullYear(), end.getUTCMonth() - months, 1))
    const lastDay = new Date(Date.UTC(firstOfTarget.getUTCFullYear(), firstOfTarget.getUTCMonth() + 1, 0)).getUTCDate()
    return new Date(Date.UTC(firstOfTarget.getUTCFullYear(), firstOfTarget.getUTCMonth(), Math.min(end.getUTCDate(), lastDay)))
  }
  return null
}
...
export function createApp(options = {}) {
  ...
  const app = express()
  app.locals.db = db
  app.locals.integrations = { studentDirectory:createStudentDirectory(), ...(options.integrations || {}) }
  ...
  app.post('/api/tenants', requirePermission('tenants.manage'), (req, res, next) => {
    try {
      ...
      const result = db.prepare(`INSERT INTO tenants(tenant_code,tenant_type,tenant_type_code,title,first_name,last_name,national_id,email,phone,current_address,faculty,department,program,major,organization,line_id,guardian_name,guardian_phone,guardian_email,guardian_line_id,emergency_contact_name,emergency_contact_phone,emergency_contact_relation,legal_entity) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(b.tenantCode,tenantTypeSystemValue(tenantTypeCode),tenantTypeCode,b.title||null,b.firstName,b.lastName,b.nationalId||null,b.email||null,b.phone||null,b.currentAddress||null,b.faculty||null,b.department||null,b.program||null,b.major||null,b.organization||null,b.lineId||null,b.guardianName||null,b.guardianPhone||null,b.guardianEmail||null,b.guardianLineId||null,b.emergencyContactName||null,b.emergencyContactPhone||null,b.emergencyContactRelation||null,b.legalEntity?1:0)
  ...
  app.patch('/api/tenants/:id', requirePermission('tenants.manage'), (req, res, next) => {
    try {
      ...
      const entries=Object.entries(b); if(entries.length){const set=entries.map(([key])=>`${map[key]}=?`).join(',');db.prepare(`UPDATE tenants SET ${set},updated_at=CURRENT_TIMESTAMP WHERE id=?`).run(...entries.map(([key,v])=>key==='legalEntity'?(v?1:0):(v??null)),id)}
  ...
  app.get('/api/integrations/students/:studentId', requirePermission('tenants.manage'), async (req,res,next)=>{try{const directory=req.app.locals.integrations.studentDirectory;if(!directory?.lookup)throw httpError(503,'INTEGRATION_NOT_CONFIGURED','ยังไม่ได้กำหนดตัวเชื่อมต่อระบบบริการการศึกษาของมหาวิทยาลัย');const record=await directory.lookup(req.params.studentId);if(!record)throw httpError(404,'STUDENT_NOT_FOUND','ไม่พบข้อมูลนักศึกษาในระบบบริการการศึกษา');const student=validate(z.object({studentCode:z.string().min(1),title:z.string().optional(),firstName:z.string().min(1),lastName:z.string().min(1),nationalId:z.string().optional(),email:z.email().optional(),phone:z.string().optional(),currentAddress:z.string().optional(),faculty:z.string().optional(),program:z.string().optional(),major:z.string().optional()}),record);writeAudit(db,req,{action:'STUDENT_DIRECTORY_LOOKUP',entityType:'student_directory',entityId:null,after:{studentCode:student.studentCode}});res.json(student)}catch(error){next(error)}})
  ...
  app.get('/api/contracts/missing', requirePermission('tenants.read'), (_req,res)=>res.json(db.prepare(`SELECT t.*,MIN(b.id) bed_id,r.room_no,MIN(b.bed_no) bed_no,bu.name building_name,(SELECT rv.condition_snapshot FROM reservations rv WHERE rv.tenant_id=t.id AND rv.room_id=r.id AND rv.status IN ('reserved','checked_in') ORDER BY rv.id DESC LIMIT 1) condition_snapshot FROM tenants t JOIN beds b ON b.tenant_id=t.id AND b.status IN ('reserved','occupied') JOIN rooms r ON r.id=b.room_id JOIN floors f ON f.id=r.floor_id JOIN buildings bu ON bu.id=f.building_id WHERE t.deleted_at IS NULL AND NOT EXISTS(SELECT 1 FROM leases l WHERE l.tenant_id=t.id AND l.status IN ('contract_pending','active','renewal_pending')) GROUP BY t.id,r.id ORDER BY t.tenant_code`).all()))
  ...
  app.post('/api/contracts', requirePermission('tenants.manage'), async (req,res,next)=>{let id=null;try{const b=validate(z.object({contractNo:z.string().min(1),tenantId:idSchema,bedId:idSchema,contractType:z.string().min(1),contractDate:z.iso.date(),rentalPeriod:z.enum(['daily','monthly','term','yearly']),startsAt:z.iso.date(),endsAt:z.iso.date(),minimumTermMonths:z.number().int().positive()}),req.body);if(b.endsAt<b.startsAt)throw httpError(400,'INVALID_DATE_RANGE','วันสิ้นสุดต้องไม่ก่อนวันเริ่มสัญญา');const locked=transaction(db,()=>{const tenant=getById(db,'tenants',b.tenantId,'AND deleted_at IS NULL'),bed=getById(db,'beds',b.bedId);if(!tenant)throw httpError(404,'TENANT_NOT_FOUND','ไม่พบผู้เช่า');if(b.contractType!==contractTypeForTenant(tenant,b.rentalPeriod))throw httpError(400,'CONTRACT_TYPE_MISMATCH','ประเภทสัญญาไม่ตรงกับประเภทผู้เช่าและรอบการเช่า');if(tenant.tenant_type==='external'&&tenant.legal_entity&&!db.prepare(`SELECT id FROM tenant_documents WHERE tenant_id=? AND document_type='company_certificate' LIMIT 1`).get(tenant.id))throw httpError(409,'COMPANY_CERTIFICATE_REQUIRED','นิติบุคคลต้องแนบหนังสือรับรองบริษัทก่อนจัดทำสัญญา');if(!bed||bed.tenant_id!==tenant.id||!['reserved','occupied'].includes(bed.status))throw httpError(409,'BED_NOT_LOCKED_FOR_TENANT','ห้องหรือเตียงไม่ได้ถูกล็อกไว้สำหรับผู้เช่ารายนี้');if(db.prepare(`SELECT id FROM leases WHERE (tenant_id=? OR bed_id=?) AND status IN ('contract_pending','active','renewal_pending')`).get(tenant.id,bed.id))throw httpError(409,'CONTRACT_SPACE_LOCKED','ผู้เช่าหรือเตียงนี้มีสัญญาที่กำลังดำเนินการอยู่แล้ว');const pricing=contractPricingFromReservation(db,tenant,bed,b.rentalPeriod),result=db.prepare(`INSERT INTO leases(contract_no,tenant_id,bed_id,contract_type,contract_date,rental_period,starts_at,ends_at,advance_rent,minimum_term_months,deposit_amount,status,document_status,rate_policy_code,rate_snapshot_json) VALUES (?,?,?,?,?,?,?,?,?,?,?,'contract_pending','draft',?,?)`).run(b.contractNo,b.tenantId,b.bedId,b.contractType,b.contractDate,b.rentalPeriod,b.startsAt,b.endsAt,pricing.amount,b.minimumTermMonths,pricing.depositAmount,pricing.ratePolicyCode,JSON.stringify(pricing));return{id:Number(result.lastInsertRowid),pricing}});id=locked.id;const contract=getContractDetails(db,id),pdf=await createContractPdf(contract),hash=sha256(pdf);transaction(db,()=>{db.prepare(`INSERT INTO contract_documents(lease_id,version,document_state,filename,pdf_data,sha256,created_by) VALUES (?,1,'draft',?,?,?,?)`).run(id,`${b.contractNo}.pdf`,pdf,hash,req.user.id);db.prepare(`UPDATE leases SET document_sha256=? WHERE id=?`).run(hash,id)});writeContractEvent(db,req,id,'CREATED',{documentHash:hash,ratePolicyCode:locked.pricing.ratePolicyCode,reservationId:locked.pricing.reservationId});const after=getContractDetails(db,id);writeAudit(db,req,{action:'CREATE',entityType:'contract',entityId:id,after});res.status(201).json(after)}catch(error){if(id&&!db.prepare(`SELECT id FROM contract_documents WHERE lease_id=?`).get(id))db.prepare(`DELETE FROM leases WHERE id=? AND document_status='draft'`).run(id);next(error)}})
  ...
  app.post('/api/contracts/:id/renew', requirePermission('tenants.manage'), async (req,res,next)=>{try{const id=numberId(req),current=getById(db,'leases',id);if(!current)throw httpError(404,'NOT_FOUND','ไม่พบสัญญา');if(current.status!=='active'||current.document_status!=='signed')throw httpError(409,'CONTRACT_NOT_RENEWABLE','ต่อสัญญาได้เฉพาะสัญญาที่ใช้งานและลงนามแล้ว');if(db.prepare(`SELECT id FROM leases WHERE previous_lease_id=? AND status='renewal_pending'`).get(id))throw httpError(409,'RENEWAL_ALREADY_EXISTS','มีฉบับต่อสัญญาที่รอดำเนินการอยู่แล้ว');const b=validate(z.object({contractNo:z.string().min(1),contractDate:z.iso.date(),startsAt:z.iso.date(),endsAt:z.iso.date(),rentalPeriod:z.enum(['monthly','term','yearly']).optional()}),req.body);if(b.startsAt<=current.ends_at)throw httpError(400,'INVALID_RENEWAL_START','วันเริ่มฉบับต่อสัญญาต้องอยู่หลังวันสิ้นสุดสัญญาปัจจุบัน');if(b.endsAt<b.startsAt)throw httpError(400,'INVALID_DATE_RANGE','วันสิ้นสุดต้องไม่ก่อนวันเริ่มสัญญา');const tenant=getById(db,'tenants',current.tenant_id),rentalPeriod=b.rentalPeriod||current.rental_period,oldSnapshot=JSON.parse(current.rate_snapshot_json||'{}'),rateScope=oldSnapshot.rateScope||(tenant.tenant_type==='external'?'room':'person'),preferredCode=rentalPeriod===current.rental_period?current.rate_policy_code:null,pricing=currentContractPricing(db,tenant,rentalPeriod,b.startsAt,rateScope,preferredCode);const result=transaction(db,()=>{db.prepare(`UPDATE leases SET renewal_requested_at=CURRENT_TIMESTAMP WHERE id=?`).run(id);return db.prepare(`INSERT INTO leases(contract_no,tenant_id,bed_id,contract_type,contract_date,rental_period,starts_at,ends_at,advance_rent,minimum_term_months,deposit_amount,status,document_status,version,previous_lease_id,rate_policy_code,rate_snapshot_json) VALUES (?,?,?,?,?,?,?,?,?,?,?,'renewal_pending','draft',?,?,?,?)`).run(b.contractNo,current.tenant_id,current.bed_id,current.contract_type,b.contractDate,rentalPeriod,b.startsAt,b.endsAt,pricing.amount,current.minimum_term_months,pricing.depositAmount,current.version+1,id,pricing.ratePolicyCode,JSON.stringify(pricing))});const newId=Number(result.lastInsertRowid),contract=getContractDetails(db,newId),pdf=await createContractPdf(contract),hash=sha256(pdf);db.prepare(`INSERT INTO contract_documents(lease_id,version,document_state,filename,pdf_data,sha256,created_by) VALUES (?,?,'draft',?,?,?,?)`).run(newId,contract.version,`${contract.contract_no}.pdf`,pdf,hash,req.user.id);db.prepare(`UPDATE leases SET document_sha256=? WHERE id=?`).run(hash,newId);writeContractEvent(db,req,id,'RENEWAL_CREATED',{newLeaseId:newId,ratePolicyCode:pricing.ratePolicyCode});writeContractEvent(db,req,newId,'CREATED_AS_RENEWAL',{previousLeaseId:id,documentHash:hash,ratePolicyCode:pricing.ratePolicyCode});writeAudit(db,req,{action:'RENEW',entityType:'contract',entityId:newId,before:current,after:contract});res.status(201).json(contract)}catch(error){next(error)}})
...
function registerRoomRoutes(app, db) {
  ...
  app.post('/api/meter-readings', requirePermission('finance.manage'), (req,res,next)=>{try{
    const b=validate(z.object({roomId:idSchema,utilityType:z.enum(['water','electricity']),billingMonth:z.string().regex(/^\d{4}-\d{2}$/),previousReading:z.number().nonnegative(),currentReading:z.number().nonnegative(),dueDate:z.iso.date(),issueInvoices:z.boolean().default(true)}).refine(x=>x.currentReading>=x.previousReading,{message:'เลขมิเตอร์ปัจจุบันต้องไม่น้อยกว่าเลขครั้งก่อน',path:['currentReading']}),req.body)
    ...
    const rate=requireUtilityRate(db,b.utilityType,`${b.billingMonth}-01`),consumption=b.currentReading-b.previousReading,total=Number(Math.max(consumption*rate.unit_rate,rate.minimum_charge).toFixed(2)),perBed=Number((total/beds.length).toFixed(2))
    const created=transaction(db,()=>{const result=db.prepare(`INSERT INTO meter_readings(room_id,utility_type,billing_month,previous_reading,current_reading,consumption,unit_rate,total_amount,divisor,amount_per_bed,due_date,invoice_issued_at,recorded_by) VALUES (?,?,?,?,?,?,?,?,?,?,?,CASE WHEN ? THEN CURRENT_TIMESTAMP END,?)`).run(b.roomId,b.utilityType,b.billingMonth,b.previousReading,b.currentReading,consumption,rate.unit_rate,total,beds.length,perBed,b.dueDate,b.issueInvoices?1:0,req.user.id);const invoiceIds=[];if(b.issueInvoices){const shares=db.prepare(`SELECT tenant_id,COUNT(*) bed_units FROM beds WHERE room_id=? AND status='occupied' AND tenant_id IS NOT NULL GROUP BY tenant_id`).all(b.roomId);for(const share of shares){const contract=signedContractForCharge(db,share.tenant_id,`${b.billingMonth}-01`);if(!contract)throw httpError(409,'SIGNED_CONTRACT_REQUIRED','ไม่สามารถตั้งหนี้ค่าสาธารณูปโภค: ผู้พักยังไม่มีสัญญาลงนามที่ครอบคลุมรอบมิเตอร์');const amount=Number((perBed*share.bed_units).toFixed(2)),invoiceNo=nextDocumentNo(db,'invoices','invoice_no','INV');const invoice=db.prepare(`INSERT INTO invoices(invoice_no,tenant_id,due_date,total,balance,contract_id,source_type,source_id,created_by) VALUES (?,?,?,?,?,?,'meter',?,?)`).run(invoiceNo,share.tenant_id,b.dueDate,amount,amount,contract.id,result.lastInsertRowid,req.user.id);db.prepare(`INSERT INTO invoice_items(invoice_id,item_type,description,quantity,unit_price,amount) VALUES (?,?,?,?,?,?)`).run(invoice.lastInsertRowid,b.utilityType,`${b.utilityType==='water'?'ค่าน้ำประปา':'ค่าไฟฟ้า'} ${b.billingMonth}`,share.bed_units,perBed,amount);invoiceIds.push(Number(invoice.lastInsertRowid))}}return{readingId:Number(result.lastInsertRowid),invoiceIds}})
...
function registerFinanceRoutes(app, db) {
  const masters = {
    ...
    'fee-types': { table:'fee_types', permission:'finance.manage', schema:z.object({code:z.string().min(1),name:z.string().min(1),defaultAmount:z.number().nonnegative().default(0),itemType:z.enum(financialItemTypes).default('other')}), columns:['code','name','default_amount','item_type'], keys:['code','name','defaultAmount','itemType'] },
  }
  for (const [path, config] of Object.entries(masters)) {
    ...
    if (path === 'fee-types') app.patch(`/api/${path}/:id`, requirePermission(config.permission), (req,res,next)=>{try{const id=numberId(req),before=getById(db,config.table,id);if(!before)throw httpError(404,'NOT_FOUND','ไม่พบรายการค่าธรรมเนียม');const b=validate(z.object({name:z.string().min(1),defaultAmount:z.number().nonnegative(),itemType:z.enum(financialItemTypes).optional(),active:z.boolean().optional()}),req.body);db.prepare(`UPDATE fee_types SET name=?,default_amount=?,item_type=?,active=? WHERE id=?`).run(b.name,b.defaultAmount,b.itemType||before.item_type,b.active===undefined?before.active:Number(b.active),id);const after=getById(db,config.table,id);writeAudit(db,req,{action:'UPDATE',entityType:config.table,entityId:id,before,after});res.json(after)}catch(error){next(error)}})
  }

  app.get('/api/revenue-share-policies', requirePermission('finance.read'), (_req,res)=>res.json(db.prepare(`SELECT * FROM revenue_share_policies ORDER BY active DESC,item_type,starts_at DESC`).all()))
  app.post('/api/revenue-share-policies', requirePermission('finance.manage'), (req,res,next)=>{try{const b=validate(z.object({code:z.string().trim().min(1).max(50),itemType:z.enum(financialItemTypes),revenueGroup:z.string().trim().min(1).max(200),revenueName:z.string().trim().min(1).max(200),reclaimRate:z.number().min(0).max(1),universityRate:z.number().min(0).max(1),startsAt:z.iso.date(),endsAt:z.iso.date().nullable().optional()}).refine(x=>Math.abs(x.reclaimRate+x.universityRate-1)<0.000001,{message:'สัดส่วนขอเบิกกลับและนำส่งมหาวิทยาลัยต้องรวมกัน 100%',path:['universityRate']}),req.body);if(b.endsAt&&b.endsAt<b.startsAt)throw httpError(400,'INVALID_DATE_RANGE','วันที่สิ้นสุดต้องไม่ก่อนวันเริ่มใช้');const result=db.prepare(`INSERT INTO revenue_share_policies(code,item_type,revenue_group,revenue_name,reclaim_rate,university_rate,starts_at,ends_at) VALUES (?,?,?,?,?,?,?,?)`).run(b.code,b.itemType,b.revenueGroup,b.revenueName,b.reclaimRate,b.universityRate,b.startsAt,b.endsAt||null);const after=getById(db,'revenue_share_policies',Number(result.lastInsertRowid));writeAudit(db,req,{action:'CREATE',entityType:'revenue_share_policy',entityId:after.id,after});res.status(201).json(after)}catch(error){next(error)}})
  app.patch('/api/revenue-share-policies/:id', requirePermission('finance.manage'), (req,res,next)=>{try{const id=numberId(req),before=getById(db,'revenue_share_policies',id);if(!before)throw httpError(404,'NOT_FOUND','ไม่พบนโยบายแบ่งรายได้');const b=validate(z.object({endsAt:z.iso.date().nullable().optional(),active:z.boolean().optional(),reason:z.string().trim().min(5).max(1000).optional()}).refine(x=>x.active!==false||x.reason,{message:'ต้องระบุเหตุผลการยกเลิก',path:['reason']}),req.body),today=bangkokDate(),cancelBeforeStart=b.active===false&&!b.endsAt&&before.starts_at>today,endsAt=b.active===false?(cancelBeforeStart?null:(b.endsAt||today)):b.endsAt;if(endsAt&&endsAt<before.starts_at)throw httpError(400,'INVALID_DATE_RANGE','วันที่สิ้นสุดต้องไม่ก่อนวันเริ่มใช้');db.prepare(`UPDATE revenue_share_policies SET ends_at=CASE WHEN ? THEN ? ELSE ends_at END,active=COALESCE(?,active),cancellation_reason=CASE WHEN ?=0 THEN ? ELSE cancellation_reason END,updated_at=CURRENT_TIMESTAMP WHERE id=?`).run(Object.hasOwn(b,'endsAt')||b.active===false?1:0,endsAt||null,Object.hasOwn(b,'active')?(b.active?1:0):null,b.active===false?0:1,b.reason||null,id);const after=getById(db,'revenue_share_policies',id);writeAudit(db,req,{action:'UPDATE',entityType:'revenue_share_policy',entityId:id,before,after});res.json(after)}catch(error){next(error)}})
  ...
  app.post('/api/invoices', requirePermission('finance.manage'), (req,res,next)=>{
    try{
      const b=validate(z.object({tenantId:idSchema,dueDate:z.iso.date(),items:z.array(z.object({feeTypeId:idSchema,description:z.string().min(1).max(500).optional(),quantity:z.number().positive().default(1)})).min(1)}),req.body)
      const contract=signedContractForCharge(db,b.tenantId,b.dueDate);if(!contract)throw httpError(409,'SIGNED_CONTRACT_REQUIRED','ต้องมีสัญญาที่ลงนามแล้วและครอบคลุมวันตั้งหนี้ก่อนออกใบแจ้งหนี้')
      const configured=b.items.map(item=>{const fee=getById(db,'fee_types',item.feeTypeId,'AND active=1');if(!fee)throw httpError(404,'FEE_TYPE_NOT_FOUND','ไม่พบค่าธรรมเนียมที่เปิดใช้งาน');if(fee.default_amount<=0)throw httpError(409,'FEE_AMOUNT_NOT_CONFIGURED',`กรุณากำหนดอัตรา ${fee.name} ใน Config ก่อนตั้งหนี้`);return{...item,itemType:fee.item_type,description:item.description||fee.name,unitPrice:fee.default_amount}})
      const id=transaction(db,()=>{const invoiceNo=nextDocumentNo(db,'invoices','invoice_no','INV');const total=Number(configured.reduce((sum,x)=>sum+x.quantity*x.unitPrice,0).toFixed(2));const result=db.prepare(`INSERT INTO invoices(invoice_no,tenant_id,due_date,total,balance,contract_id,source_type,created_by) VALUES (?,?,?,?,?,?,'manual',?)`).run(invoiceNo,b.tenantId,b.dueDate,total,total,contract.id,req.user.id);for(const item of configured)db.prepare(`INSERT INTO invoice_items(invoice_id,item_type,description,quantity,unit_price,amount) VALUES (?,?,?,?,?,?)`).run(result.lastInsertRowid,item.itemType,item.description,item.quantity,item.unitPrice,Number((item.quantity*item.unitPrice).toFixed(2)));return Number(result.lastInsertRowid)});const after=getById(db,'invoices',id);writeAudit(db,req,{action:'CREATE',entityType:'invoice',entityId:id,after});res.status(201).json({...after,items:db.prepare(`SELECT * FROM invoice_items WHERE invoice_id=?`).all(id)})
    }catch(error){next(error)}
  })
  ...
  app.post('/api/payments', requirePermission('finance.manage'), (req,res,next)=>{
    try{
      ...
      if(b.method!=='cash'&&b.referenceNo&&db.prepare(`SELECT id FROM payments WHERE method=? AND reference_no=?`).get(b.method,b.referenceNo))throw httpError(409,'DUPLICATE_PAYMENT_REFERENCE','เลขอ้างอิงการชำระถูกใช้แล้ว')
  ...
  app.post('/api/receipts/:id/cancel', requirePermission('finance.cancel'), (req,res,next)=>{
    try{const id=numberId(req),before=getById(db,'receipts',id);if(!before)throw httpError(404,'NOT_FOUND','ไม่พบใบเสร็จ');if(before.status==='cancelled')throw httpError(409,'ALREADY_CANCELLED','ใบเสร็จถูกยกเลิกแล้ว');if(db.prepare(`SELECT 1 FROM remittance_receipts rr JOIN remittances rm ON rm.id=rr.remittance_id WHERE rr.receipt_id=? AND rm.status IN ('submitted','approved')`).get(id))throw httpError(409,'REMITTANCE_MUST_BE_CANCELLED_FIRST','ใบเสร็จถูกนำส่งแล้ว ต้องยกเลิกรายการนำส่งก่อน');const{reason}=validate(z.object({reason:z.string().min(5).max(1000)}),req.body);transaction(db,()=>{const payment=getById(db,'payments',before.payment_id),invoice=getById(db,'invoices',payment.invoice_id);db.prepare(`UPDATE receipts SET status='cancelled',cancelled_reason=?,cancelled_by=?,cancelled_at=CURRENT_TIMESTAMP WHERE id=?`).run(reason,req.user.id,id);const balance=invoice.balance+payment.amount;db.prepare(`UPDATE invoices SET balance=?,status=? WHERE id=?`).run(balance,balance>=invoice.total?'issued':'partial',invoice.id)});const after=getById(db,'receipts',id);writeAudit(db,req,{action:'CANCEL',entityType:'receipt',entityId:id,before,after});res.json(after)}catch(error){next(error)}
  })
  ...
  app.post('/api/finance/assess-late-fees', requirePermission('finance.manage'), (req,res,next)=>{try{
    const {asOf}=validate(z.object({asOf:z.iso.date()}),req.body),feeDefault=Number(db.prepare(`SELECT default_amount FROM fee_types WHERE code='LATE_FEE' AND active=1`).get()?.default_amount||0)
    const overdue=db.prepare(`SELECT i.*,t.tenant_type FROM invoices i JOIN tenants t ON t.id=i.tenant_id WHERE i.status IN ('issued','partial') AND i.balance>0 AND date(i.due_date)<date(?) AND i.late_fee_assessed_at IS NULL AND i.source_type!='late_fee' ORDER BY i.id`).all(asOf)
    const created=[]
    transaction(db,()=>{for(const invoice of overdue){const contract=getById(db,'leases',invoice.contract_id);if(!contract)continue;const amount=contractFinanceRules(contract,invoice.tenant_type,feeDefault).lateFee;if(amount<=0){db.prepare(`UPDATE invoices SET late_fee_assessed_at=? WHERE id=?`).run(asOf,invoice.id);continue}const invoiceNo=nextDocumentNo(db,'invoices','invoice_no','INV'),result=db.prepare(`INSERT INTO invoices(invoice_no,tenant_id,due_date,total,balance,contract_id,source_type,source_id,created_by) VALUES (?,?,?,?,?,?,'late_fee',?,?)`).run(invoiceNo,invoice.tenant_id,asOf,amount,amount,contract.id,invoice.id,req.user.id);db.prepare(`INSERT INTO invoice_items(invoice_id,item_type,description,quantity,unit_price,amount) VALUES (?,'late_fee',?,1,?,?)`).run(result.lastInsertRowid,`ค่าปรับชำระล่าช้าของใบแจ้งหนี้ ${invoice.invoice_no}`,amount,amount);db.prepare(`UPDATE invoices SET late_fee_assessed_at=? WHERE id=?`).run(asOf,invoice.id);created.push({id:Number(result.lastInsertRowid),invoice_no:invoiceNo,source_invoice_id:invoice.id,amount})}})
    for(const item of created)writeAudit(db,req,{action:'LATE_FEE_ASSESSED',entityType:'invoice',entityId:item.id,after:item})
    res.status(201).json({asOf,createdCount:created.length,invoices:created})
  }catch(error){next(error)}})

  app.get('/api/finance/delinquency', requirePermission('finance.read'), (req,res,next)=>{try{
    if(req.user.tenant_id)throw httpError(403,'FORBIDDEN','ข้อมูลติดตามหนี้สำหรับเจ้าหน้าที่เท่านั้น')
    const asOf=validate(z.iso.date(),req.query.asOf||new Date().toISOString().slice(0,10)),rows=db.prepare(`SELECT i.id,i.invoice_no,i.tenant_id,i.due_date,i.balance,i.contract_id,t.tenant_code,t.tenant_type,t.first_name,t.last_name,GROUP_CONCAT(DISTINCT ii.item_type) item_types FROM invoices i JOIN tenants t ON t.id=i.tenant_id JOIN invoice_items ii ON ii.invoice_id=i.id WHERE i.status IN ('issued','partial') AND i.balance>0 AND date(i.due_date)<date(?) GROUP BY i.id ORDER BY i.due_date`).all(asOf),groups=new Map()
    for(const row of rows){const utility=row.item_types.split(',').some(type=>['water','electricity'].includes(type));if(row.tenant_type==='student'&&!utility)continue;const list=groups.get(row.tenant_id)||[];list.push(row);groups.set(row.tenant_id,list)}
    const at=new Date(`${asOf}T00:00:00Z`),result=[]
    for(const list of groups.values()){const first=list[0],contract=getById(db,'leases',first.contract_id),rules=contractFinanceRules(contract,first.tenant_type),months=new Set(list.map(x=>x.due_date.slice(0,7))),oldest=list.reduce((min,x)=>x.due_date<min?x.due_date:min,list[0].due_date),oldestDate=new Date(`${oldest}T00:00:00Z`),ageMonths=(at.getUTCFullYear()-oldestDate.getUTCFullYear())*12+at.getUTCMonth()-oldestDate.getUTCMonth(),thresholdReached=first.tenant_type==='student'?months.size>=rules.delinquencyMonths:ageMonths>=rules.delinquencyMonths
      result.push({tenant_id:first.tenant_id,tenant_code:first.tenant_code,tenant_type:first.tenant_type,tenant_name:`${first.first_name} ${first.last_name}`,outstanding_balance:Number(list.reduce((sum,x)=>sum+x.balance,0).toFixed(2)),overdue_invoice_count:list.length,overdue_months:months.size,oldest_due_date:oldest,threshold_months:rules.delinquencyMonths,threshold_reached:thresholdReached,required_action:thresholdReached?rules.terminationAction:'ติดตามและแจ้งเตือนการชำระ'})}
    res.json({asOf,rows:result.sort((a,b)=>Number(b.threshold_reached)-Number(a.threshold_reached)||b.outstanding_balance-a.outstanding_balance)})
  }catch(error){next(error)}})
  ...
  app.post('/api/bank-imports', requirePermission('finance.manage'), (req,res,next)=>{
    try{
      ...
      const importResult=db.prepare(`INSERT INTO bank_imports(bank_code,filename,row_count,success_count,error_count,imported_by) VALUES (?,?,?,0,0,?)`).run(b.bankCode,b.filename,lines.length,req.user.id),importId=Number(importResult.lastInsertRowid);let success=0;const errors=[]
      for(let index=0;index<lines.length;index++){const values=lines[index].split(',').map(x=>x.trim()),row=Object.fromEntries(headers.map((h,i)=>[h,values[i]])),amount=Number(row.amount);try{const invoice=db.prepare(`SELECT * FROM invoices WHERE invoice_no=?`).get(row.invoice_no);if(!invoice||invoice.status==='cancelled')throw new Error('ไม่พบใบแจ้งหนี้');if(!Number.isFinite(amount)||amount<=0||amount>invoice.balance)throw new Error('ยอดเงินไม่ถูกต้อง');if(row.reference_no&&db.prepare(`SELECT id FROM payments WHERE method='bank_file' AND reference_no=?`).get(row.reference_no))throw new Error('เลขอ้างอิงธนาคารซ้ำ');const ids=transaction(db,()=>{const p=db.prepare(`INSERT INTO payments(invoice_id,amount,method,reference_no,paid_at,received_by) VALUES (?,?,'bank_file',?,?,?)`).run(invoice.id,amount,row.reference_no,row.paid_at,req.user.id),receipt=db.prepare(`INSERT INTO receipts(receipt_no,payment_id,issued_by) VALUES (?,?,?)`).run(nextDocumentNo(db,'receipts','receipt_no','RC'),p.lastInsertRowid,req.user.id),balance=invoice.balance-amount;db.prepare(`UPDATE invoices SET balance=?,status=? WHERE id=?`).run(balance,balance===0?'paid':'partial',invoice.id);db.prepare(`INSERT INTO bank_import_rows(bank_import_id,row_no,invoice_no,amount,reference_no,paid_at,status,invoice_id,payment_id,receipt_id) VALUES (?,?,?,?,?,?,'matched',?,?,?)`).run(importId,index+2,row.invoice_no,amount,row.reference_no,row.paid_at,invoice.id,p.lastInsertRowid,receipt.lastInsertRowid);return{paymentId:Number(p.lastInsertRowid),receiptId:Number(receipt.lastInsertRowid)}});success++;writeAudit(db,req,{action:'BANK_ROW_MATCHED',entityType:'bank_import_row',entityId:`${importId}:${index+2}`,after:{invoiceId:invoice.id,amount,referenceNo:row.reference_no,...ids}})}catch(error){errors.push({line:index+2,message:error.message});db.prepare(`INSERT INTO bank_import_rows(bank_import_id,row_no,invoice_no,amount,reference_no,paid_at,status,error_message) VALUES (?,?,?,?,?,?,'error',?)`).run(importId,index+2,row.invoice_no,Number.isFinite(amount)?amount:null,row.reference_no,row.paid_at,error.message)}}
      db.prepare(`UPDATE bank_imports SET success_count=?,error_count=? WHERE id=?`).run(success,errors.length,importId);writeAudit(db,req,{action:'BANK_IMPORT',entityType:'bank_import',entityId:importId,after:{filename:b.filename,rowCount:lines.length,success,errorCount:errors.length}});res.status(201).json({id:importId,rowCount:lines.length,successCount:success,errorCount:errors.length,errors})
    }catch(error){next(error)}
  })
  ...
  app.post('/api/remittances', requirePermission('finance.manage'), (req,res,next)=>{try{const b=validate(z.object({date:z.iso.date(),holdingStatementAmount:z.number().nonnegative(),cashDepositReference:z.string().max(200).optional()}),req.body);const duplicate=db.prepare(`SELECT id FROM remittances WHERE remittance_date=? AND status!='cancelled'`).get(b.date);if(duplicate)throw httpError(409,'DUPLICATE_REMITTANCE','มีรายการนำส่งของวันนี้แล้ว');const receipts=db.prepare(`SELECT r.id receipt_id,p.amount,p.method,i.id invoice_id,i.total FROM receipts r JOIN payments p ON p.id=r.payment_id JOIN invoices i ON i.id=p.invoice_id WHERE r.status='issued' AND date(p.paid_at)=? AND NOT EXISTS(SELECT 1 FROM remittance_receipts rr JOIN remittances existing ON existing.id=rr.remittance_id WHERE rr.receipt_id=r.id AND existing.status!='cancelled') ORDER BY r.id`).all(b.date),total=Number(receipts.reduce((sum,row)=>sum+row.amount,0).toFixed(2));if(total<=0)throw httpError(409,'NO_UNREMITTED_RECEIPTS','ไม่พบใบเสร็จที่รอนำส่งในวันที่เลือก');const cash=Number(receipts.filter(x=>x.method==='cash').reduce((sum,row)=>sum+row.amount,0).toFixed(2));if(cash>0&&!b.cashDepositReference)throw httpError(400,'CASH_DEPOSIT_REFERENCE_REQUIRED','กรุณาระบุเลขอ้างอิงการนำฝากเงินสดเข้าบัญชีพัก');const difference=Number((b.holdingStatementAmount-total).toFixed(2));if(Math.abs(difference)>=0.01)throw httpError(409,'RECONCILIATION_MISMATCH',`ยอดบัญชีพักต่างจากยอดใบเสร็จ ${difference.toFixed(2)} บาท`);let deposit=0;for(const receipt of receipts)deposit+=db.prepare(`SELECT COALESCE(SUM(ii.amount*?/i.total),0) amount FROM invoices i JOIN invoice_items ii ON ii.invoice_id=i.id WHERE i.id=? AND ii.item_type='deposit'`).get(receipt.amount,receipt.invoice_id).amount;deposit=Number(deposit.toFixed(2));const transfer=Number((total-cash).toFixed(2)),no=nextDocumentNo(db,'remittances','remittance_no','REM'),id=transaction(db,()=>{const result=db.prepare(`INSERT INTO remittances(remittance_no,remittance_date,revenue_amount,deposit_amount,cash_amount,transfer_amount,holding_statement_amount,reconciliation_difference,cash_deposit_reference,reconciled_by,reconciled_at,created_by) VALUES (?,?,?,?,?,?,?,?,?,?,CURRENT_TIMESTAMP,?)`).run(no,b.date,Number((total-deposit).toFixed(2)),deposit,cash,transfer,b.holdingStatementAmount,difference,b.cashDepositReference||null,req.user.id,req.user.id);const insert=db.prepare(`INSERT INTO remittance_receipts(remittance_id,receipt_id,amount) VALUES (?,?,?)`);for(const receipt of receipts)insert.run(result.lastInsertRowid,receipt.receipt_id,receipt.amount);return Number(result.lastInsertRowid)});const after=getById(db,'remittances',id);writeAudit(db,req,{action:'RECONCILED_AND_CREATED',entityType:'remittance',entityId:after.id,after:{...after,receiptCount:receipts.length}});res.status(201).json({...after,receipt_count:receipts.length})}catch(error){next(error)}})
...
function registerSupportRoutes(app, db) {
  ...
  app.post('/api/inventory', requirePermission('inventory.manage'), (req,res,next)=>{try{const b=validate(z.object({sku:z.string().min(1),name:z.string().min(1),category:z.enum(['dormitory','maintenance','cleaning','other']),unit:z.string().min(1),quantity:z.number().nonnegative().default(0),reorderLevel:z.number().nonnegative().default(0)}),req.body);const id=transaction(db,()=>{const result=db.prepare(`INSERT INTO inventory_items(sku,name,category,unit,quantity,reorder_level) VALUES (?,?,?,?,?,?)`).run(b.sku,b.name,b.category,b.unit,b.quantity,b.reorderLevel);if(b.quantity>0)db.prepare(`INSERT INTO inventory_movements(item_id,movement_type,quantity,reference,performed_by) VALUES (?,'adjust',?,'ยอดเริ่มต้น',?)`).run(result.lastInsertRowid,b.quantity,req.user.id);return Number(result.lastInsertRowid)});const after=getById(db,'inventory_items',id);writeAudit(db,req,{action:'CREATE',entityType:'inventory',entityId:after.id,after});res.status(201).json(after)}catch(error){next(error)}})
