...
function registerFinanceRoutes(app, db) {
  const masters = {
    ...
    'fee-types': { table:'fee_types', permission:'finance.manage', schema:z.object({code:z.string().min(1),name:z.string().min(1),defaultAmount:z.number().nonnegative().default(0),itemType:z.enum(['room','water','electricity','deposit','late_fee','damage','food_beverage','other']).default('other')}), columns:['code','name','default_amount','item_type'], keys:['code','name','defaultAmount','itemType'] },
  }
  for (const [path, config] of Object.entries(masters)) {
    ...
    if (path === 'fee-types') app.patch(`/api/${path}/:id`, requirePermission(config.permission), (req,res,next)=>{try{const id=numberId(req),before=getById(db,config.table,id);if(!before)throw httpError(404,'NOT_FOUND','ไม่พบรายการค่าธรรมเนียม');const b=validate(z.object({name:z.string().min(1),defaultAmount:z.number().nonnegative(),itemType:z.enum(['room','water','electricity','deposit','late_fee','damage','food_beverage','other']).optional(),active:z.boolean().optional()}),req.body);db.prepare(`UPDATE fee_types SET name=?,default_amount=?,item_type=?,active=? WHERE id=?`).run(b.name,b.defaultAmount,b.itemType||before.item_type,b.active===undefined?before.active:Number(b.active),id);const after=getById(db,config.table,id);writeAudit(db,req,{action:'UPDATE',entityType:config.table,entityId:id,before,after});res.json(after)}catch(error){next(error)}})
  }
