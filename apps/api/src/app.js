...
function registerFinanceRoutes(app, db) {
  ...
  app.patch('/api/revenue-share-policies/:id', requirePermission('finance.manage'), (req,res,next)=>{try{const id=numberId(req),before=getById(db,'revenue_share_policies',id);if(!before)throw httpError(404,'NOT_FOUND','ไม่พบนโยบายแบ่งรายได้');const b=validate(z.object({endsAt:z.iso.date().nullable().optional(),active:z.boolean().optional(),reason:z.string().trim().min(5).max(1000).optional()}).refine(x=>x.active!==false||x.reason,{message:'ต้องระบุเหตุผลการยกเลิก',path:['reason']}),req.body),endsAt=b.active===false?(b.endsAt||new Date().toISOString().slice(0,10)):b.endsAt;if(endsAt&&endsAt<before.starts_at)throw httpError(400,'INVALID_DATE_RANGE','วันที่สิ้นสุดต้องไม่ก่อนวันเริ่มใช้');db.prepare(`UPDATE revenue_share_policies SET ends_at=CASE WHEN ? THEN ? ELSE ends_at END,active=COALESCE(?,active),cancellation_reason=CASE WHEN ?=0 THEN ? ELSE cancellation_reason END,updated_at=CURRENT_TIMESTAMP WHERE id=?`).run(Object.hasOwn(b,'endsAt')||b.active===false?1:0,endsAt||null,Object.hasOwn(b,'active')?(b.active?1:0):null,b.active===false?0:1,b.reason||null,id);const after=getById(db,'revenue_share_policies',id);writeAudit(db,req,{action:'UPDATE',entityType:'revenue_share_policy',entityId:id,before,after});res.json(after)}catch(error){next(error)}})
