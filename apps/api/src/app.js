...
import { emailConfigured, sendContractSignatureEmail } from './email.js'
...
const contractDetailSql = `SELECT l.*,t.tenant_code,t.tenant_type,t.title,t.first_name,t.last_name,t.national_id,
  ...
  FROM leases l JOIN tenants t ON t.id=l.tenant_id LEFT JOIN beds b ON b.id=l.bed_id
...
export function createApp(options = {}) {
  ...
  const tenantBody = z.object({ tenantCode: z.string().min(1).max(50), tenantType: z.enum(['student','staff','external']), title: z.string().max(30).optional(), firstName: z.string().min(1), lastName: z.string().min(1), nationalId: z.string().max(30).optional(), email: z.email().optional(), phone: z.string().max(30).optional(), currentAddress: z.string().max(1000).optional(), faculty: z.string().max(200).optional(), department: z.string().max(200).optional(), program:z.string().max(200).optional(),major:z.string().max(200).optional(),organization:z.string().max(300).optional(),lineId:z.string().max(100).optional(),guardianName:z.string().max(200).optional(),guardianPhone:z.string().max(30).optional(),guardianEmail:z.email().optional(),guardianLineId:z.string().max(100).optional(),emergencyContactName:z.string().max(200).optional(),emergencyContactPhone:z.string().max(30).optional(),emergencyContactRelation:z.string().max(100).optional() })
  app.get('/api/tenants', requirePermission('tenants.read'), (_req, res) => res.json(db.prepare(`SELECT t.*,r.room_no,b.bed_no,bu.name building_name,l.id lease_id,l.contract_no,l.starts_at contract_starts_at,l.ends_at contract_ends_at,l.document_status contract_document_status FROM tenants t LEFT JOIN beds b ON b.tenant_id=t.id AND b.status IN ('reserved','occupied') LEFT JOIN rooms r ON r.id=b.room_id LEFT JOIN floors f ON f.id=r.floor_id LEFT JOIN buildings bu ON bu.id=f.building_id LEFT JOIN leases l ON l.tenant_id=t.id AND l.status='active' WHERE t.deleted_at IS NULL GROUP BY t.id ORDER BY t.id DESC`).all()))
  app.post('/api/tenants', requirePermission('tenants.manage'), (req, res, next) => {
    try {
      const b = validate(tenantBody, req.body)
      const result = db.prepare(`INSERT INTO tenants(tenant_code,tenant_type,title,first_name,last_name,national_id,email,phone,current_address,faculty,department,program,major,organization,line_id,guardian_name,guardian_phone,guardian_email,guardian_line_id,emergency_contact_name,emergency_contact_phone,emergency_contact_relation) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(b.tenantCode,b.tenantType,b.title||null,b.firstName,b.lastName,b.nationalId||null,b.email||null,b.phone||null,b.currentAddress||null,b.faculty||null,b.department||null,b.program||null,b.major||null,b.organization||null,b.lineId||null,b.guardianName||null,b.guardianPhone||null,b.guardianEmail||null,b.guardianLineId||null,b.emergencyContactName||null,b.emergencyContactPhone||null,b.emergencyContactRelation||null)
  ...
  app.post('/api/tenants/:id/portal-account', requirePermission('users.manage'), async (req,res,next)=>{
    try{
      ...
      db.prepare(`INSERT OR IGNORE INTO role_permissions(role_id,permission_id) SELECT ?,id FROM permissions WHERE code IN ('repairs.read','repairs.create','contracts.read','contracts.sign')`).run(role.id)
  ...
  app.get('/api/contracts/alerts', requirePermission('tenants.read'), (req,res,next)=>{try{const asOf=validate(z.iso.date(),req.query.asOf||new Date().toISOString().slice(0,10)),today=new Date(`${asOf}T00:00:00Z`);const rows=db.prepare(`${contractDetailSql} WHERE l.status='active' AND l.document_status='signed' AND date(l.ends_at)>=date(?)`).all(asOf);const alerts=rows.filter(row=>{const days=Math.ceil((new Date(`${row.ends_at}T00:00:00Z`)-today)/86400000);return row.rental_period==='monthly'?(today.getUTCDate()>=25&&days<=40):row.rental_period==='term'?days<=30:row.rental_period==='yearly'?days<=60:false}).map(row=>({...row,days_remaining:Math.ceil((new Date(`${row.ends_at}T00:00:00Z`)-today)/86400000)}));res.json(alerts)}catch(error){next(error)}})
  ...
  app.post('/api/contracts/:id/renew', requirePermission('tenants.manage'), async (req,res,next)=>{try{const id=numberId(req),current=getById(db,'leases',id);if(!current)throw httpError(404,'NOT_FOUND','ไม่พบสัญญา');const b=validate(z.object({contractNo:z.string().min(1),contractDate:z.iso.date(),startsAt:z.iso.date(),endsAt:z.iso.date(),rentalPeriod:z.enum(['monthly','term','yearly']).optional()}),req.body);const result=transaction(db,()=>{db.prepare(`UPDATE leases SET renewal_requested_at=CURRENT_TIMESTAMP WHERE id=?`).run(id);return db.prepare(`INSERT INTO leases(contract_no,tenant_id,bed_id,contract_type,contract_date,rental_period,starts_at,ends_at,advance_rent,minimum_term_months,deposit_amount,document_status,version,previous_lease_id) VALUES (?,?,?,?,?,?,?,?,?,?,?,'draft',?,?)`).run(b.contractNo,current.tenant_id,current.bed_id,current.contract_type,b.contractDate,b.rentalPeriod||current.rental_period,b.startsAt,b.endsAt,current.advance_rent,current.minimum_term_months,current.deposit_amount,current.version+1,id)});const newId=Number(result.lastInsertRowid),contract=getContractDetails(db,newId),pdf=await createContractPdf(contract),hash=sha256(pdf);db.prepare(`INSERT INTO contract_documents(lease_id,version,document_state,filename,pdf_data,sha256,created_by) VALUES (?,?,'draft',?,?,?,?)`).run(newId,contract.version,`${contract.contract_no}.pdf`,pdf,hash,req.user.id);db.prepare(`UPDATE leases SET document_sha256=? WHERE id=?`).run(hash,newId);writeContractEvent(db,req,id,'RENEWAL_CREATED',{newLeaseId:newId});writeContractEvent(db,req,newId,'CREATED_AS_RENEWAL',{previousLeaseId:id,documentHash:hash});writeAudit(db,req,{action:'RENEW',entityType:'contract',entityId:newId,before:current,after:contract});res.status(201).json(contract)}catch(error){next(error)}})
...
function registerFinanceRoutes(app, db) {
  ...
  app.get('/api/invoices', requirePermission('finance.read'), (req,res)=>{
    const tenantFilter=req.user.tenant_id?'WHERE i.tenant_id=?':'';const rows=db.prepare(`SELECT i.*,t.tenant_code,t.first_name,t.last_name FROM invoices i JOIN tenants t ON t.id=i.tenant_id ${tenantFilter} ORDER BY i.id DESC`).all(...(req.user.tenant_id?[req.user.tenant_id]:[]));res.json(rows)
  })
  ...
  app.get('/api/receipts', requirePermission('finance.read'), (req,res)=>{const filter=req.user.tenant_id?'WHERE i.tenant_id=?':'';res.json(db.prepare(`SELECT r.*,p.amount,p.method,p.paid_at,i.invoice_no,i.tenant_id FROM receipts r JOIN payments p ON p.id=r.payment_id JOIN invoices i ON i.id=p.invoice_id ${filter} ORDER BY r.id DESC`).all(...(req.user.tenant_id?[req.user.tenant_id]:[])))})
